...
print("Decoupling jefe_reportes from admin3_reportes...")
...
# Rename class
content = content.replace('class Admin3Reportes(QWidget):', 'class JefeReportes(QWidget):')
content = content.replace('Admin3Reportes', 'JefeReportes')

# Fix Historial SQL and infinite scroll
hist_func = """    def _cargar_historial_tickets(self):
...
# Use regex to replace the entire _cargar_historial_tickets block
match = re.search(r'(    def _cargar_historial_tickets\(self\):.*?)(?=\n    def )', content, flags=re.DOTALL)
if match:
    content = content.replace(match.group(1), hist_func)
    print("Replaced _cargar_historial_tickets logic.")
else:
    print("WARNING: Could not find _cargar_historial_tickets with regex. Checking alternative regex.")
    match2 = re.search(r'(    def _cargar_historial_tickets\(self\):.*?)(?=\Z)', content, flags=re.DOTALL)
    if match2:
        content = content.replace(match2.group(1), hist_func)
        print("Replaced _cargar_historial_tickets at EOF.")

# Save to jefe_reportes.py
with codecs.open('src/jefe/jefe_reportes.py', 'w', encoding='utf-8') as f:
    f.write(content)
print("Saved to src/jefe/jefe_reportes.py")

# Also fix the SQL in admin3_reportes.py just in case the admin opens it
with codecs.open('src/admin/admin3_reportes.py', 'r', encoding='utf-8') as f:
    admin_content = f.read()
admin_content = admin_content.replace('dv.id_producto = p.codigo', 'dv.id_producto = p.id')
with codecs.open('src/admin/admin3_reportes.py', 'w', encoding='utf-8') as f:
    f.write(admin_content)
print("Patched admin3_reportes SQL condition.")
