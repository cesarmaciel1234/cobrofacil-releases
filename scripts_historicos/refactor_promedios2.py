...
start_idx = content.find('    def _build_tab_promedios(self):')
end_idx = content.find('    def _load_promedios(self):')

if start_idx == -1 or end_idx == -1:
    print("Could not find blocks")
    exit(1)

new_code = """    def _build_tab_promedios(self):
        lay, _ = self._page()
        ...
        self._prom_tabla = build_table(["Corte", "Kilos", "Costo $/kg", "% Ganancia", "Precio/kg Venta", "Oferta", "Venta Total", "Ganancia Neta"])
    ...
    def _guardar_estado_actual(self):
        ...
        for r in range(self._prom_tabla.rowCount()):
            row_data = [self._prom_tabla.item(r, c).text() if self._prom_tabla.item(r, c) else "" for c in range(8)]
    ...
    def _cambiar_tipo_promedio(self, tipo):
        ...
        active_style = f"background: {PAL['primary']}; color: white; border-radius: 8px; padding: 12px 24px; font-weight: bold; font-size: 14px;"
        inactive_style = f"background: #E2E8F0; color: #0F172A; border: 1px solid #94A3B8; border-radius: 8px; padding: 12px 24px; font-weight: bold; font-size: 14px;"
