...
with open('src/jefe/jefe_contabilidad.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Start block
start_idx = content.find("    # TAB 0 — RESUMEN FINANCIERO")

# End block
end_idx = content.find("    # TAB 1 — INGRESOS")
if end_idx != -1:
    # Find the horizontal rule before it
...
new_code = """    # TAB 0 — RESUMEN FINANCIERO (DASHBOARD CFO)
    # ─────────────────────────────────────────────────────────────────────────
    def _build_tab_resumen(self):
        ...
        pnl_card = QFrame()
        pnl_card.setStyleSheet(f"QFrame {{ background: {PAL['surface']}; border-radius: 12px; border: 1px solid {PAL['border']}; }}")
        ...
        exp_card = QFrame()
        exp_card.setStyleSheet(f"QFrame {{ background: {PAL['surface']}; border-radius: 12px; border: 1px solid {PAL['border']}; }}")
    ...
    def _create_kpi_pro(self, title, value, color, subtitle):
        ...
        lbl_t = QLabel(title)
        lbl_t.setStyleSheet(f"color: {PAL['text3']}; font-weight: bold; font-size: 11px; letter-spacing: 1px; background: transparent; border: none;")
        l.addWidget(lbl_t)
        ...
        lbl_s.setStyleSheet(f"color: {PAL['text2']}; font-size: 11px; background: transparent; border: none;")
        l.addWidget(lbl_s)
...
with open('src/jefe/jefe_contabilidad.py', 'w', encoding='utf-8') as f:
    f.write(new_content)
