...
class DatabaseManager:
    ...
    def _nombre_producto_para_db(self, nombre):
        ...
        if getattr(self, "db_engine_type", "sqlite") == "mariadb" or getattr(
            self, "mariadb_engine", None
        ):
    ...
    def sync_venta_to_master(self, venta_data, items):
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                def _txt(value):
                    nombre = self._nombre_producto_para_db(value)
                    if force_sanitize:
                        from src.db_engines.mariadb_engine import mariadb_safe_text
                        nombre = mariadb_safe_text(nombre)
                    return nombre
                ...
                """, (
                    ...
                    venta_data['estado'], venta_data['metodo_pago'], fecha_local, c_id,
                    venta_data.get('descuento', 0.0), venta_data.get('recargo', 0.0),
                    _txt(venta_data.get('cliente_nombre', ''))
                ))
                id_venta = cursor.lastrowid
                ...
                for it in items:
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?)
                    """, (
                        id_venta, it.get('id', ''), _txt(self._item_nombre(it)),
                ...
                if (
                    is_mariadb
                    and not force_sanitize
                    and self._is_mariadb_encoding_error(e)
                ):
                    force_sanitize = True
                    logger.warning(
                        "sync_venta_to_master: reintento con nombres sin emojis tras error 1366"
                    )
                    continue
                if attempt < max_attempts - 1 and is_mariadb and self._is_transient_mariadb_error(e):
                    self._reset_mariadb_thread_connection()
