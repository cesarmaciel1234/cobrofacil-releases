...
class DatabaseManager:
    ...
    def _nombre_producto_para_db(self, nombre):
        ...
        if getattr(self, "db_engine_type", "sqlite") == "mariadb" or getattr(self, "mariadb_engine", None):
            from src.db_engines.mariadb_engine import mariadb_safe_text
            return mariadb_safe_text(nombre)
        return nombre or ""

    def _texto_para_mariadb(self, value):
        """Texto genérico seguro para columnas MariaDB utf8 (cliente_nombre, etc.)."""
        if getattr(self, "db_engine_type", "sqlite") == "mariadb" or getattr(self, "mariadb_engine", None):
            from src.db_engines.mariadb_engine import mariadb_safe_text
            return mariadb_safe_text(value)
        return value or ""
    ...
    def _get_mariadb_connection_strict(self):
        """Conexión MariaDB sin fallback a SQLite (sync offline hacia maestra)."""
        engine = getattr(self, "mariadb_engine", None)
        if getattr(self, "db_engine_type", "sqlite") != "mariadb" or not engine:
            raise RuntimeError("MariaDB no disponible para sync hacia maestra")
    ...
    def guardar_venta_completa(self, venta_data, items):
        ...
        try:
            ...
            """, (
                ...
                self._texto_para_mariadb(venta_data.get('cliente_nombre', ''))
            ))
    ...
    def sync_venta_to_master(self, venta_data, items):
        ...
        for attempt in range(max_attempts):
            conn = None
            try:
                conn = self._get_mariadb_connection_strict() if is_mariadb else self.get_connection()
                cursor = conn.cursor()
                ...
                for it in items:
                    ...
                    """, (
                        id_venta, it.get('id', ''),
                        self._nombre_producto_para_db(self._item_nombre(it)),
                        it.get('cant', 1), it.get('precio', 0), it.get('subtotal', 0),
                    ))

                    if it.get('id') and str(it['id']).strip() not in ('000', ''):
                        cursor.execute(
                            "UPDATE productos SET stock = stock - ? WHERE id = ?",
                            (it.get('cant', 1), it.get('id')),
                        )
                ...
                if conn:
                    try:
                        ...
                        pass
                retryable = is_mariadb and (
                    self._is_transient_mariadb_error(e) or self._is_mariadb_encoding_error(e)
                )
                if attempt < max_attempts - 1 and retryable:
                    logger.warning(
                        "sync_venta_to_master reintento %s/%s: %s",
                        attempt + 1,
                        max_attempts,
                        e,
                    )
