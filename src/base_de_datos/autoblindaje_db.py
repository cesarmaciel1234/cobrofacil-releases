...
class AutoBlindajeDB:
    ...
    def auto_reparar_o_restaurar(cls, engine_type: str, host: str = "127.0.0.1") -> bool:
        ...
        if engine_type == "mariadb":
            # Corrupción aislada en clientes (1877): recrear tabla sin restore completo
            if cls._mariadb_table_is_corrupt(host, "clientes"):
                logger.warning(
                    "Corrupción InnoDB en clientes; recreando tabla antes de restore completo..."
                )
                if cls._recrear_tabla_clientes_mariadb(host) and cls._check_mariadb_integrity(host):
                    return True
