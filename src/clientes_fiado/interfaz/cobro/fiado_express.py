...
class DialogoFiadoExpressPaso1(_FiadoExpressBase):
    ...
    def _cargar(self):
        ...
        dni_norm = cerebro.normalizar_dni(self.txt_dni.text())
        if not dni_norm:
        ...
        cliente, estado, msg = cerebro.identificar_dni(self.txt_dni.text())
        self.btn_cargar.setEnabled(True)
        self.btn_cargar.setText("CARGAR")
        ...
        if cerebro.limite_excedido(cliente, self.monto_total):
...
class DialogoFiadoExpressConfirmacion(_FiadoExpressBase):
    ...
    def _build(self):
        ...
        disp = cerebro.credito_disponible(self.cliente)
    ...
    def _verificar(self):
        ...
        dni2 = cerebro.normalizar_dni(self.txt_dni.text())
        if not dni2:
