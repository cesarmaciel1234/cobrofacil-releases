...
`panel.py`, clase `PanelMixtoCobro`. `mostrar(total)` pinta efectivo, tarjeta, transferencia y QR. `ocultar()` lo saca. `valores()` es el reparto. `cubre()` es verdadero cuando la suma alcanza el total. `cambio` avisa cada vez que se escribe.
