...
`toast.py`, clase `AvisoCobro`. `mostrar(mensaje)` lo pinta sobre la fila de redondeo y a los pocos segundos se va solo. `pin(mensaje, marcas)` es el mismo cartel en rojo, con cuatro círculos, y se queda hasta que el cobro lo cierra: lo usa el fiado cuando el cupo no alcanza. El mouse lo atraviesa: el cajero sigue escribiendo. Si llega `accion`, aparece el botón «Asociar», Enter lo acepta y el cartel se queda hasta esa decisión. El texto, si el monto no es el del ticket, es que llegó ese importe y se puede asociar o esperar otro. `alarma(mensaje)` es el cartel rojo con el signo de alarma: Espere la transferencia, el QR o la tarjeta. Con el TPV apagado ese bloqueo no corre y Enter registra.
