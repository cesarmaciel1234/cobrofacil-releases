...
class DialogoHistorialDia(QDialog):
    def __init__(self, parent=None, is_embedded=False, perfil="cajero"):
        ...
        if self.is_embedded and hasattr(self, 'btn_close'):
            self.btn_close.hide()
    ...
    def setup_ui(self):
        ...
        # SEGURIDAD PARA EL JEFE (Modo Lectura Estricta)
        if self.perfil == "jefe":
            self.btn_cancelar.hide()
            self.btn_notas.hide()
            self.tabla_tickets.setFocusPolicy(Qt.NoFocus)
            self.tabla_detalle.setFocusPolicy(Qt.NoFocus)
    ...
    def keyPressEvent(self, event):
        ...
        if getattr(self, 'perfil', 'cajero') == "jefe":
            forbidden_keys = [Qt.Key_Return, Qt.Key_Enter, Qt.Key_Up, Qt.Key_Down, Qt.Key_Left, Qt.Key_Right]
