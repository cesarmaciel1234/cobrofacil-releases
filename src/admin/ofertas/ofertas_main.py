...
from src.admin.ofertas.componentes.tabla_ofertas import TablaOfertas
from src.admin.ofertas.componentes.creador_promociones import CreadorPromociones
...
class Admin2Ofertas(QWidget):
    ...
    def _on_activar_promo(self, data):
        ...
        ok = db_manager.execute_non_query(
            ...
            (data['cant_oferta'], data['precio_oferta'], data['precio_oferta_relampago'], data['precio_oferta_promedio'], data['limite_oferta_relampago'], data['id'])
        )
        if ok:
            self.cargar_datos()
    ...
    def _on_imprimir_cartel_rapido(self, id_p, cant_oferta, precio_oferta):
        rows = db_manager.execute_query("SELECT * FROM productos WHERE id=?", (id_p,))
        if not rows: return
        p = dict(rows[0])
        ...
        if qt_exec(dlg):
            try:
                ...
                try: from services.etiquetas.renderer import EtiquetaRenderer, abrir_archivo_pdf
    ...
    def _imprimir_cartelera_masiva(self):
        ...
        rows = db_manager.execute_query(f"SELECT * FROM productos WHERE id IN ({placeholders})", tuple(self.checked_product_ids)) or []
        
        for p in rows:
            if float(p['cant_oferta'] or 0.0) > 0 and float(p['precio_oferta'] or 0.0) > 0:
        ...
        if qt_exec(dlg):
            try: from src.services.etiquetas.renderer import EtiquetaRenderer, abrir_archivo_pdf
    ...
    def _crear_folleto_pdf(self):
        rows_db = db_manager.execute_query("SELECT * FROM productos WHERE cant_oferta > 0 AND precio_oferta > 0 ORDER BY departamento, nombre") or []
        ...
        if qt_exec(dlg):
            ...
            if rb_sel.isChecked():
                placeholders = ",".join(["?"] * len(self.checked_product_ids))
                rows_filtradas = db_manager.execute_query(f"SELECT * FROM productos WHERE id IN ({placeholders})", tuple(self.checked_product_ids)) or []
    ...
    def _configurar_ofertas_secuencial(self):
        ...
        placeholders = ",".join(["?"] * len(self.checked_product_ids))
        seleccionados = db_manager.execute_query(f"SELECT * FROM productos WHERE id IN ({placeholders})", tuple(self.checked_product_ids)) or []
