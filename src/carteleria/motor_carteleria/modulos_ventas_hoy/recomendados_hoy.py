...
class ModuloRecomendadoHoy:
    ...
    def obtener_productos_para_cartel(self, limit=10):
        ...
        if len(lista_pantalla) < 5:
            ...
            # Consultar productos en stock de la base general o carteleria para sugerir hoy (sin ORDER BY RAND)
            rows_sug = db_manager.execute_query("SELECT nombre FROM productos WHERE stock > 0")
            if rows_sug:
