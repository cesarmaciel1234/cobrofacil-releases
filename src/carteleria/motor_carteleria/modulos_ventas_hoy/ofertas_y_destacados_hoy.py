...
class ModuloOfertasYDestacadosHoy:
    ...
    def obtener_productos_para_cartel(self, limit=10):
        ...
        if len(nombres_encontrados) < limit:
            fetch_limit = limit * 2
            q_prod = "SELECT nombre FROM productos WHERE precio_oferta > 0 ORDER BY nombre LIMIT ?"
            rows_p = db_manager.execute_query(q_prod, (fetch_limit,))
            if rows_p:
                rows_p = random.sample(list(rows_p), min(fetch_limit, len(rows_p)))
            if rows_p:
                for r in rows_p:
