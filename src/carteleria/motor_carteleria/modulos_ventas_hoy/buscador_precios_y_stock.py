...
class BuscadorDePreciosYStock:
    ...
    def armar_lista_para_pantalla(productos_vendidos_hoy):
        ...
        for prod in productos_vendidos_hoy:
            ...
            # a) Intentamos leer de 'carteleria_global' (match exacto primero: TRIM/LOWER no usa índice en MariaDB)
            ...
            if not rows_cartel:
                ...
                rows_cartel = db_manager.execute_query(q_cartel, (nombre_busqueda,))
            if not rows_cartel:
                q_cartel = "SELECT precio_normal, precio_oferta, regla_texto FROM carteleria_global WHERE LOWER(nombre_producto) LIKE LOWER(?)"
                rows_cartel = db_manager.execute_query(q_cartel, (f"%{nombre_busqueda}%",))
            ...
            rows_prod = db_manager.execute_query(
            ...
            if not rows_prod:
                ...
                rows_prod = db_manager.execute_query(q_prod, (nombre_busqueda,))
            if not rows_prod:
                q_prod = "SELECT precio, precio_oferta, stock, unidad FROM productos WHERE LOWER(nombre) LIKE LOWER(?)"
                rows_prod = db_manager.execute_query(q_prod, (f"%{nombre_busqueda}%",))
