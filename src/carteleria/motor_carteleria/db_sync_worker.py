    ...
    def run(self):
        try:
            ...
            try:
                ...
                # 2. SOS (Soporta múltiples ofertas relámpago rotativas; sin ORDER BY RAND: timeout MariaDB 2013)
                sos_pool_q = (
                    ...
                    "AND LOWER(nombre) NOT LIKE '%articulo comun%' AND LOWER(nombre) NOT LIKE '%venta libre%' LIMIT 200"
                )
                sos_pool = db_manager.execute_query(sos_pool_q) or []
                oferta_sos = random.sample(list(sos_pool), min(10, len(sos_pool))) if sos_pool else []
                ...
                fallback_pool_q = (
                    ...
                    "WHERE precio > 0 AND LOWER(nombre) NOT LIKE '%articulo comun%' "
                    "AND LOWER(nombre) NOT LIKE '%venta libre%' LIMIT 200"
                )
                fallback_pool = db_manager.execute_query(fallback_pool_q) or []
                top_dict["hoy"] = random.sample(list(fallback_pool), min(10, len(fallback_pool))) if fallback_pool else []
