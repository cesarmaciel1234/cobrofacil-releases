...
class SharedRedLanPanel(QWidget):
    ...
    def __init__(self, parent=None, show_admin_buttons=True):
    ...
    def _build_card_cambiar_modo(self) -> QFrame:
        ...
        desc_m = QLabel(
            ...
            "potente (MariaDB). Actuará como el servidor\n"
            "y cerebro para las terminales esclavas."
        )
        ...
        self.combo_ip_maestra = QComboBox()
        self.combo_ip_maestra.setEditable(True)
        self.combo_ip_maestra.lineEdit().setPlaceholderText("IP de la Maestra  (ej: 192.168.0.100)")
        self.combo_ip_maestra.setStyleSheet(
    ...
    def _actualizar_botones(self):
        ...
        if is_master:
            ...
            self.btn_hacer_maestra.setEnabled(True)
            self.btn_hacer_maestra.setText("✅ Convertir en MAESTRA")
            self.btn_hacer_maestra.setToolTip(
                "Requiere MariaDB / Servidor de Tienda en ESTA PC.\n"
                "En una TV de cartelería no arranca mysqld (evita congelar)."
            )
            self.btn_hacer_esclava.setEnabled(True)
            self.btn_hacer_esclava.setText("🔗 Reconectar ESCLAVA")

    def _convertir_maestra(self):
        """Pasa esta PC a modo MAESTRA si hay MariaDB local."""
    ...
    def _convertir_esclava(self):
        ...
        idx = self.combo_ip_maestra.currentIndex()
        if idx >= 0 and self.combo_ip_maestra.itemData(idx):
