...
from src.motor_descuentos.ofertas.tabla import TablaOfertas
from src.motor_descuentos.ofertas.editor import CreadorPromociones


def _id_de_celda(item):
    if not item:
        return ""
    raw = item.data(Qt.ItemDataRole.UserRole)
    if raw not in (None, ""):
        return str(raw)
    m = re.search(r"\[(\d+)\]", item.text() or "")
    return m.group(1) if m else (item.text() or "").strip()


class _HiloBusquedaOfertas(QThread):
    ...
    def __init__(self, parent=None):
        ...
        self.buscar = ""
        self.depto = None
        self.solo_promos = False

    def setup(self, buscar, depto, solo_promos):
        self.buscar = buscar
        self.depto = depto
        self.solo_promos = solo_promos

    def run(self):
        try:
            motor = MotorOfertas()
            filas = motor.buscar_productos(self.buscar, self.depto or "", self.solo_promos) or []
            self.listo.emit([dict(r) for r in filas])
        except Exception:
            self.listo.emit([])
...
class TallerOfertas(QWidget):
    ...
    def __init__(self):
        ...
        self.all_rows = []
        self.loaded_count = 0
        self._hilo = _HiloBusquedaOfertas(self)
        self._hilo.listo.connect(self._on_busqueda_listo)
    ...
    def _setup_ui(self):
        ...
        self.setStyleSheet(
            "QWidget#TallerOfertas { background: #F8FAFC; }"
            "QWidget#TallerOfertas QDoubleSpinBox, QWidget#TallerOfertas QLineEdit {"
    ...
    def cargar_datos(self):
        ...
        if self._hilo.isRunning():
            self._busqueda_pendiente = (buscar, depto, solo_promos)
            return
        self.lbl_total.setText("🔄 Cargando...")
        self._hilo.setup(buscar, depto, solo_promos)
        self._hilo.start()

    def _on_busqueda_listo(self, filas):
        pendiente = self._busqueda_pendiente
        self._busqueda_pendiente = None
        if pendiente:
            self._hilo.setup(*pendiente)
            self._hilo.start()
            return
        self.all_rows = filas
        self.loaded_count = 0
        ...
        sin_stock = sum(1 for r in self.all_rows if float(r.get('stock') or 0.0) <= 0)
        n = len(self.all_rows)
        extra = " (máx. 800)" if n >= 800 else ""
        self.lbl_total.setText(f"📦 {n} PRODUCTOS EN LISTA{extra}")
    ...
    def _cargar_panel_fila(self, row):
        ...
        if not p:
            try:
                p = MotorOfertas().obtener_producto(id_p)
            except Exception:
                p = None
        if p:
            self.panel_control.cargar_producto(p)
    ...
    def _on_activar_promo(self, data):
        ...
        if ok:
            ...
            for r in self.all_rows:
                if str(r.get('id')) == pid:
                    r['cant_oferta'] = data['cant_oferta']
                    r['precio_oferta'] = data['precio_oferta']
            ...
            self.lbl_sel.setText("Oferta guardada")
        else:
        ...
        if ok:
            ...
            for r in self.all_rows:
                if str(r.get('id')) == pid:
                    r['cant_oferta'] = 0
                    r['precio_oferta'] = 0
                    r['precio_oferta_relampago'] = 0
                    r['precio_oferta_promedio'] = 0
                    break
            self.loaded_count = 0
            self.tabla.setRowCount(0)
            self._cargar_siguiente_pagina()
            self.tabla.select_product_by_id(id_p)
            self.lbl_sel.setText("Oferta quitada")
        else:
