...
from src.utils.text_db import sanitize_mariadb_params, safe_mariadb_text, ascii_mariadb_text
...
def mariadb_ascii_text(value, max_len=None):
    """Fallback ASCII para error 1366 en columnas utf8 legacy."""
    text = ascii_mariadb_text(value)
    if max_len is not None:
        text = text[:max_len]
    return text
