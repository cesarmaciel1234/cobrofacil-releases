...
class Paso7CierreCaja(QDialog):
    ...
    def _get_data(self):
        ...
        res_v = db_manager.execute_query(
            "SELECT SUM(total) as t, SUM(pago_otro) as ta FROM ventas WHERE estado='COMPLETADA' AND LOWER(usuario) = LOWER(?) AND date(fecha) = ?",
            (self.user, today_str)
        )
        ...
        if v_total == 0:
            res_v_fallback = db_manager.execute_query(
                "SELECT SUM(total) as t, SUM(pago_otro) as ta FROM ventas WHERE estado='COMPLETADA' AND date(fecha) = ?",
                (today_str,)
            )
        ...
        res_d = db_manager.execute_query(
            "SELECT SUM(total) as t, SUM(pago_otro) as ta FROM ventas WHERE date(fecha) = ? AND estado IN ('COMPLETADA','CERRADA')",
            (today_str,)
        )
        ...
        return {
            "fondo": fondo, "t_efec": v_total - v_tarj, "t_tarj": v_tarj, "t_total": v_total,
            "d_total": d_total, "d_tarj": d_tarj, "f_hist": f_hist, "alertas": alertas,
            "esperado": fondo + (v_total - v_tarj)
        }
    ...
    def setup_ui(self):
        ...
        self.main_container.setStyleSheet("QFrame#MainContainer { background: #F8FAFC; border-radius: 20px; border: 1px solid #E2E8F0; }")
        ...
        header.setStyleSheet("QFrame#HeaderFrame { background: #1E3A8A; border-top-left-radius: 19px; border-top-right-radius: 19px; border: none; }")
        h_lay = QHBoxLayout(header)
        h_lay.setContentsMargins(30, 0, 30, 0)
        
        lbl_tit = QLabel("💎 COBROFACIL - CONTROL DE CIERRE")
        lbl_tit.setStyleSheet("color: white; font-size: 14px; font-weight: 900; letter-spacing: 2px; border: none; background: transparent;")
        h_lay.addWidget(lbl_tit)
        h_lay.addStretch()
        
        if self.datos["f_hist"] > 0:
            self.lbl_sos = QLabel(f"⚠️ DEUDA: $ {self.datos['f_hist']:,.2f}")
            self.lbl_sos.setStyleSheet("color: #F87171; font-weight: 900; font-size: 14px; background: rgba(239, 68, 68, 0.1); padding: 5px 15px; border-radius: 8px; border: none;")
            h_lay.addWidget(self.lbl_sos)
            h_lay.addStretch()

        # Icono de usuario blanco brillante
        lbl_u_icon = QLabel()
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        user_icon_path = os.path.join(base_dir, "assets", "usuario_blanco.png")
        if os.path.exists(user_icon_path):
            pixmap = QPixmap(user_icon_path)
            lbl_u_icon.setPixmap(pixmap.scaled(18, 18, Qt.KeepAspectRatio, Qt.SmoothTransformation))
            lbl_u_icon.setStyleSheet("border: none; background: transparent;")
        else:
            lbl_u_icon.setText("👤")
            lbl_u_icon.setStyleSheet("color: white; font-size: 13px; border: none; background: transparent;")
        h_lay.addWidget(lbl_u_icon)

        lbl_u = QLabel(f"{self.user.upper()} | {datetime.now().strftime('%H:%M')}")
        lbl_u.setStyleSheet("color: #FFFFFF; font-weight: 800; font-size: 12px; border: none; background: transparent;")
        ...
        for c in [self.card_efec, self.card_tarj, self.card_fondo, self.card_alertas]: v_met.addWidget(c)
        ...
        footer.setStyleSheet("QFrame#FooterFrame { background: #F8FAFC; border-top: 1px solid #E2E8F0; border-bottom-left-radius: 19px; border-bottom-right-radius: 19px; border: none; }")
        ...
        if self.role == "admin":
            self.btn_global = QPushButton("🌍 CIERRE TOTAL GLOBAL")
            self.btn_global.setFixedSize(220, 50)
            self.btn_global.setStyleSheet("background: #3B82F6; color: white; border-radius: 12px; font-weight: 900; font-size: 12px;")
            self.btn_global.clicked.connect(self.finalizar_global_accion)
            f_lay.addWidget(self.btn_global)
            
            self.btn_reporte = QPushButton("📊 REPORTE ADMIN")
            self.btn_reporte.setFixedSize(180, 50)
            self.btn_reporte.setStyleSheet("background: #F1F5F9; color: #1E3A8A; border-radius: 12px; font-weight: 900; font-size: 12px;")
            self.btn_reporte.clicked.connect(self.mostrar_corte_diario)
            f_lay.addWidget(self.btn_reporte)
    ...
    def finalizar_turno_accion(self):
        now = datetime.now()
        last_click = getattr(self, '_last_click_time', None)
        if last_click and (now - last_click).total_seconds() < 1.5:
            return
    ...
    def finalizar_global_accion(self):
        now = datetime.now()
        last_click = getattr(self, '_last_click_time', None)
        if last_click and (now - last_click).total_seconds() < 1.5:
            return
        ...
        if self.role != 'admin':
            QMessageBox.warning(self, "Acceso Denegado", "Solo los administradores pueden registrar un CIERRE GLOBAL.")
            return
        self.modo = "total"
        self._finalizar()
    ...
    def _finalizar(self):
        ...
        if self.modo == "total":
            msg = "¿Está seguro de que desea registrar el CIERRE GLOBAL de todas las cajas?\n\nEsta acción cerrará las ventas globales de todos los cajeros e imprimirá el reporte físico."
        ...
        try:
            ...
            if self.modo == "turno": db_manager.execute_non_query("UPDATE ventas SET estado='CERRADA' WHERE estado='COMPLETADA' AND usuario=? AND caja_id=?", (self.user, c_id))
            else: db_manager.execute_non_query("UPDATE ventas SET estado='CERRADA' WHERE estado='COMPLETADA' AND caja_id=?", (c_id,))
            
            from src.hardware.printer import printer_manager
            printer_manager.imprimir_ticket_z(self.user, fisico, dif, {**self.datos, "segunda_tiketera": True})
            QMessageBox.information(self, "CIERRE EXITOSO", f"Cierre registrado correctamente.\nDiferencia: $ {dif:,.2f}")
