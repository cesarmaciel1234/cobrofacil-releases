...
class Admin7Cierre(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.lbl_hdr_vivo_titulo = QLabel("💎 COBROFACIL - CONTROL DE CIERRE")
        ...
        # Contenedor para el usuario y el icono a la derecha
        usr_widget = QWidget()
        usr_widget.setStyleSheet("background: transparent; border: none;")
        usr_lay = QHBoxLayout(usr_widget)
        usr_lay.setContentsMargins(0, 0, 0, 0)
        usr_lay.setSpacing(6)
        
        self.lbl_u_icon = QLabel()
        ...
        if os.path.exists(user_icon_path):
            ...
            self.lbl_u_icon.setPixmap(pixmap.scaled(16, 16, Qt.KeepAspectRatio, Qt.SmoothTransformation))
            self.lbl_u_icon.setStyleSheet("border: none; background: transparent;")
        else:
            self.lbl_u_icon.setText("👤")
            self.lbl_u_icon.setStyleSheet("color: white; font-size: 11px; border: none; background: transparent;")
        usr_lay.addWidget(self.lbl_u_icon)
        
        self.lbl_hdr_vivo_usuario = QLabel("ADMIN")
        self.lbl_hdr_vivo_usuario.setStyleSheet("color: #FFFFFF; font-weight: 800; font-size: 11px; background: transparent;")
        usr_lay.addWidget(self.lbl_hdr_vivo_usuario)
        
        lay_hdr_v.addWidget(usr_widget, 0, Qt.AlignRight)
    ...
    def seleccionar_caja(self, key_caja):
        ...
        self.lbl_hdr_vivo_titulo.setText(f"💎 COBROFACIL - CONTROL DE CIERRE: CAJA 0{c_id} ({cajero_actual.upper()})")
