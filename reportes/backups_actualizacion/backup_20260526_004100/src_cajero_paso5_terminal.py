...
class Paso5Terminal(QWidget):
    ...
    def setup_ui(self):
        ...
        if HAS_KEYBOARD:
            ...
            self.btn_teclado.setFixedHeight(30)
        ...
        self.lbl_version = QLabel("🚀 COBROFACIL ELITE v2026.0 | TERMINAL ACTIVA")
        ...
        icon_lbl = QLabel("⌨️")
        icon_lbl.setStyleSheet("color: #94A3B8; font-size: 11px; border: none; background: transparent; padding-right: 4px;")
        ...
        self.btn_candado.setFixedSize(35, 30)
    ...
    def toggle_keyboard(self):
        ...
        has_kb = hasattr(self, 'teclado_virtual')
        if has_kb:
            try:
                _ = self.teclado_virtual.parent()
            except RuntimeError:
                delattr(self, 'teclado_virtual')
                has_kb = False
    ...
    def on_focus_changed(self, old_widget, new_widget):
        ...
        if new_widget and isinstance(new_widget, QLineEdit):
            ...
            try:
                ...
                tactil_setting = config.get("pantalla_tactil", "auto")
                is_tactil = False
                if isinstance(tactil_setting, bool):
                    is_tactil = tactil_setting
                elif str(tactil_setting).lower() == "true":
                    is_tactil = True
                elif str(tactil_setting).lower() == "false":
                    is_tactil = False
                else:
                    try:
                        import ctypes
                        digitizer = ctypes.windll.user32.GetSystemMetrics(94)
                        is_tactil = bool(digitizer & (0x80 | 0x40))
                    except Exception:
                        is_tactil = False

                # Si es táctil, o si el teclado ya estaba visible (apertura manual)
                if is_tactil or (hasattr(self, 'teclado_virtual') and self.teclado_virtual.isVisible()):
                    self.teclado_virtual.reposition_keyboard()
                    self.teclado_virtual.show()
                else:
                    self.teclado_virtual.hide()
    ...
    def _actualizar_barra_cajero(self):
        ...
        if CajeroActivo.numero == 2:
            self.lbl_version.setText(f"🟢 {nombre_str}  |  COBROFACIL ELITE v2026.0")
            self.lbl_version.setStyleSheet("color: #10b981; font-weight: 900; font-size: 11px; letter-spacing: 1px; border: none;")
            self.btn_candado.setStyleSheet("""
                QPushButton {
    ...
    def abrir_cierre_caja(self):
        # Evitar cerrar si hay una venta activa (productos en el carrito)
        if self.tabla.rowCount() > 0:
            from PyQt5.QtWidgets import QMessageBox
            QMessageBox.warning(
                self, "Venta en Progreso",
                "No se puede realizar el cierre de caja mientras haya una venta en progreso.\n\n"
                "Por favor, finalice o limpie la venta actual antes de proceder al cierre."
            )
            return
