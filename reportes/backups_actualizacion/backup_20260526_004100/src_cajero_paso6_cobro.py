...
# Teclado virtual flotante no se utiliza en cobro. Paso6Cobro usa build_teclado_propio.
...
class Paso6Cobro(QDialog):
    ...
    def setup_ui(self):
        ...
        self.ultimo_foco = self.txt_pago
        ...
        self.txt_otro = QLineEdit("")
        self.txt_otro.setPlaceholderText("0.00")
    ...
    def calcular_vuelto(self):
        try:
            ...
            p1_t = self.txt_pago.text().replace('$', '').strip()
            p2_t = self.txt_otro.text().replace('$', '').strip()
            p1 = parse_float_regional(p1_t)
            p2 = parse_float_regional(p2_t) if self.current_metodo == "Mixto" else 0
            total_pagado = p1 + p2
    ...
    def _validar_pago(self):
        ...
        try:
            p1 = parse_float_regional(p1_t)
            p2 = parse_float_regional(p2_t) if p2_t and self.current_metodo == "Mixto" else 0
            ...
            QMessageBox.critical(self, "ERROR", f"Monto inválido ingresado: {e}")
            return None
    ...
    def eventFilter(self, watched, event):
        ...
        if watched in [self.txt_pago, self.txt_otro, self.txt_desc, self.txt_rec]:
            if event.type() == QEvent.FocusIn:
                self.ultimo_foco = watched
    ...
    def build_teclado_propio(self, right_lay):
        from src.ui_components.virtual_keyboard_paso6 import VirtualKeyboardPaso6
        self.teclado = VirtualKeyboardPaso6(self)
        right_lay.addWidget(self.teclado)
