...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("CobroFacil Elite 2026 - Industrial POS")
    ...
    def _handle_f12_logic(self):
        ...
        elif self.stacked_widget.currentIndex() == 0 and config.current_user.get('role') == 'admin':
            self.switch_tab(6)
        # Si ya estamos en Auditoría/Cierre Z, iniciar el diálogo de cierre Z
        elif self.stacked_widget.currentIndex() == 6 and config.current_user.get('role') == 'admin':
            if hasattr(self.screens[6], '_ejecutar_cierre_z_dialogo'):
                self.screens[6]._ejecutar_cierre_z_dialogo()
