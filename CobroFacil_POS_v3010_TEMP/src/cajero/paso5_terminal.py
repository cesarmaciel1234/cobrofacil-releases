...
class _BotCmdSignal(QObject):
    """Señales Qt para recibir comandos del bot desde el hilo UDP de forma segura."""
    add_product = pyqtSignal(str)
    scan_text   = pyqtSignal(str)
...
class Paso5Terminal(QWidget):
    ...
    def __init__(self):
        ...
        # UDP listener para comandos del bot asistente (puerto 45679)
        self._start_bot_udp_listener()

    def _start_bot_udp_listener(self):
        """Escucha comandos UDP del bot para agregar productos al carrito sin pyautogui."""
        import threading, socket as _sock
        self._bot_cmd_signal = _BotCmdSignal()
        self._bot_cmd_signal.add_product.connect(self._handle_bot_add_product)
        self._bot_cmd_signal.scan_text.connect(self._handle_bot_scan_text)
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        if msg.startswith('BOT_ADD_PRODUCT:'):
                            codigo = msg[len('BOT_ADD_PRODUCT:'):].strip()
                            self._bot_cmd_signal.add_product.emit(codigo)
                        elif msg.startswith('BOT_SCAN_TEXT:'):
                            txt = msg[len('BOT_SCAN_TEXT:'):].strip()
                            self._bot_cmd_signal.scan_text.emit(txt)
                    except _sock.timeout:
                        continue
                    except Exception:
                        break
            except Exception as e:
                print(f'[PASO5 UDP] Error: {e}')
        threading.Thread(target=_listen, daemon=True).start()

    def _handle_bot_add_product(self, codigo):
        """Agrega producto al carrito directamente desde comando del bot."""
        try:
            res = db_manager.execute_query(
                "SELECT id, nombre, precio, cant_oferta, precio_oferta, unidad "
                "FROM productos WHERE codigo = ? OR LOWER(nombre) LIKE ? LIMIT 1",
                (codigo, f"%{codigo.lower()}%")
            )
            if res:
                self.agregar_a_tabla(res[0], 1.0)
                print(f'[PASO5 UDP] Producto agregado: {res[0]["nombre"]}')
            else:
                print(f'[PASO5 UDP] No encontrado: {codigo}')
        except Exception as e:
            print(f'[PASO5 UDP] Error agregando: {e}')

    def _handle_bot_scan_text(self, txt):
        """Pone texto en el scanner y procesa (equivale a escribir + Enter)."""
        try:
            if self.txt_scan:
                self.txt_scan.setText(txt)
                self.txt_scan.setFocus()
                self.procesar_scan()
        except Exception as e:
            print(f'[PASO5 UDP] Error scan_text: {e}')
    ...
    def keyPressEvent(self, event):
        ...
        elif k == Qt.Key_F10: self.toggle_chatbot()
    ...
    def toggle_chatbot(self):
        if getattr(self, 'chatbot_widget', None) is None:
            try:
                from src.cajero.chat_bot import ChatManualWidget
                self.chatbot_widget = ChatManualWidget(self.window())
                # Posicionar a la derecha por defecto
                if self.window():
                    geom = self.window().geometry()
                    self.chatbot_widget.move(geom.right() - 480, geom.bottom() - 700)
            except Exception as e:
                import logging
                logging.getLogger(__name__).error(f"Error cargando chatbot: {e}")
                return
        
        if self.chatbot_widget.isVisible():
            self.chatbot_widget.hide()
            self.txt_scan.setFocus()
        else:
            self.chatbot_widget.show()
            self.chatbot_widget.raise_()
            self.chatbot_widget.activateWindow()
