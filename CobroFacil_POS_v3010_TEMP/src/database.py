            ...
            if self.db_engine_type == "mariadb":
                ...
                if not self.is_master:
                    try:
                        ...
                        msg.setWindowTitle("⚠️ CONEXIÓN AL CEREBRO PERDIDA")
                        msg.setText(f"Esta caja no puede encontrar a la PC Maestra (IP: {host}).\n\nVerifica que la PC Maestra esté encendida y conectada a la red Wi-Fi.")
                        
                        btn_retry = msg.addButton("🔄 Reintentar Conexión", QMessageBox.AcceptRole)
                        btn_local = msg.addButton("🖥️ ARRANCAR SERVIDOR LOCAL AISLADO (Modo Maestra)", QMessageBox.RejectRole)
                        
                        msg.exec_()
                        
                        if msg.clickedButton() == btn_local:
                            config.set("db_host", "") # Volver a maestra local
                            config.save()
                            logger.info("Cambiando a modo MAESTRA aislado por decisión del usuario (Offline Fallback).")
                            app.exit(888)
                            import sys
                            sys.exit(888)
                        else:
