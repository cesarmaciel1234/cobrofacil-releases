...
class Admin16LANConnection(QWidget):
    ...
    def setup_ui(self):
        ...
        # Header
        header = QLabel("🔌 CONECTAR SERVIDOR MULTICAJA (MARIADB)")
        header.setStyleSheet("font-size: 26px; font-weight: bold; color: #1e293b;")
        main_layout.addWidget(header)
        
        # --- SECCION 1: ESTADO ACTUAL ---
        status_frame = QFrame()
        status_frame.setStyleSheet("border-radius: 12px; padding: 20px;")
        status_lay = QVBoxLayout(status_frame)
        
        if getattr(db_manager, "is_master", True):
            # Es MAESTRA
            status_frame.setStyleSheet(status_frame.styleSheet() + "background-color: #ecfdf5; border: 2px solid #10b981;")
            title = QLabel("👑 ESTA PC ES EL CEREBRO (MAESTRA)")
            title.setStyleSheet("font-size: 20px; font-weight: bold; color: #059669; border: none;")
            
            desc = QLabel("Todas las ventas y configuraciones se guardan físicamente en el disco duro de esta computadora.\nPara conectar otras Cajas (Esclavas), usa estos datos:")
            desc.setStyleSheet("font-size: 14px; color: #047857; border: none;")
            
            ip_lbl = QLabel(f"IP LOCAL: {get_local_ip()}")
            ip_lbl.setStyleSheet("font-size: 32px; font-weight: bold; color: #1e293b; background: white; padding: 10px; border-radius: 8px; border: 1px solid #a7f3d0;")
            ip_lbl.setAlignment(Qt.AlignCenter)
            
            port_lbl = QLabel("PUERTO: 3306")
            port_lbl.setStyleSheet("font-size: 20px; font-weight: bold; color: #047857; border: none;")
            ...
            status_frame.setStyleSheet(status_frame.styleSheet() + "background-color: #eff6ff; border: 2px solid #3b82f6;")
            title = QLabel("🖥️ MODO CAJA CONECTADA (ESCLAVA)")
            title.setStyleSheet("font-size: 20px; font-weight: bold; color: #1d4ed8; border: none;")
            
            desc = QLabel(f"Esta PC NO guarda datos en su disco duro. Está conectada al Cerebro (Maestra) en la IP:")
            desc.setStyleSheet("font-size: 14px; color: #1e40af; border: none;")
            
            ip_lbl = QLabel(f"{host_conectado}")
            ip_lbl.setStyleSheet("font-size: 32px; font-weight: bold; color: #1e293b; background: white; padding: 10px; border-radius: 8px; border: 1px solid #bfdbfe;")
            ip_lbl.setAlignment(Qt.AlignCenter)
        ...
        scan_frame = QFrame()
        scan_frame.setStyleSheet("background: white; border: 1px solid #cbd5e1; border-radius: 10px; padding: 20px;")
        scan_lay = QVBoxLayout(scan_frame)
        
        lbl_scan_title = QLabel("📡 Radar de Servidores Activos en la Red")
        lbl_scan_title.setStyleSheet("font-size: 16px; font-weight: bold; color: #334155; border: none;")
        scan_lay.addWidget(lbl_scan_title)
        
        self.btn_scan = QPushButton("🔍 BUSCAR CEREBROS (MAESTRAS) EN RED")
        ...
        self.lbl_scan_status.setStyleSheet("color: #64748b; font-size: 13px; border: none;")
        ...
        manual_frame = QFrame()
        manual_frame.setStyleSheet("background: white; border: 1px solid #cbd5e1; border-radius: 10px; padding: 20px;")
        manual_lay = QVBoxLayout(manual_frame)
        
        lbl_manual = QLabel("Conexión Manual por IP:")
        lbl_manual.setStyleSheet("font-size: 14px; font-weight: bold; color: #1e293b; border: none;")
        manual_lay.addWidget(lbl_manual)
        
        self.txt_ip = QLineEdit()
        self.txt_ip.setPlaceholderText("Ejemplo: 192.168.0.15")
        self.txt_ip.setStyleSheet("padding: 10px; font-size: 16px; border: 1px solid #94a3b8; border-radius: 5px;")
        ...
        if not getattr(db_manager, "is_master", True):
            self.btn_reset = QPushButton("👑 VOLVER A SER PC MAESTRA")
